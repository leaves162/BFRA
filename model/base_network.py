...
class affine_part(nn.Module):
    ...
    def __init__(self, planes):
        ...
        self.plane=planes
        self.affine_gamma = nn.Parameter(torch.ones(1, planes))
        self.affine_beta = nn.Parameter(torch.zeros(1, planes))
    def forward(self, x):
        gamma = self.affine_gamma.view(1, -1, 1, 1)
        beta = self.affine_beta.view(1, -1, 1, 1)
        return gamma * x + beta

class BaseNetwork_affine(nn.Module):
    expansion = 1
    def __init__(self, inplanes, planes, stride=1, downsample=None, num_classes=None):
        """
        Args:
    ...
    def forward(self, x):#由于nn.Sequential只接受单输入，所以x=[x, num_samples=None]
        #print('forward_type:', type(x))
        identity = x[0]
        xx=x[0]
        #print('inital x:',identity.shape)
        num_samples=x[1]
        #num_samples=[448,64,64,64,64,64,64,64]
        out = self.conv1(xx)
        #print('conv1:',out.shape)
        out = self.bn1(out)
        #print('x before affine1:',out.shape)
        if self.num_classes==None or num_samples==None:
            out = self.affine1(out)
            #print('x after affine1:', out.shape)
        else:
            #print('out:', out.shape)
            #print(num_samples)
            out_list = list(torch.split(out, num_samples))
            out=[]
            for t in range(len(self.num_classes)):
                out.append(self.affine1[t](out_list[t]))
            out2=self.affine1[-1](torch.cat(out_list, dim=0))
            out=torch.cat(out, dim=0)+out2
            #print(out.shape)
        out = self.relu(out)
        ...
        out = self.bn2(out)
        #print('x before affine2:',out.shape)
        if self.num_classes == None or num_samples==None:
            out = self.affine2(out)
            #print('x after affine2:',out.shape)
        else:
            out_list = list(torch.split(out, num_samples))
            out = []
            for t in range(len(self.num_classes)):
                out.append(self.affine2[t](out_list[t]))
            #self.affine2[-1](torch.cat(out_list, dim=0))
...
class ResNet_MDL_affine(nn.Module):
    def __init__(self, block, layers, num_classes=64,
        ...
        self.initial_pool = False
        self.affine_normalize = affine_part(3)
        inplanes = self.inplanes = 64
        self.num_classes = num_classes
    ...
    def _make_layer(self, block, planes, blocks, stride=1):
        ...
        layers.append(block(self.inplanes, planes, stride, downsample=downsample,num_classes=temp_num_classes))
        self.inplanes = planes * block.expansion
        for _ in range(1, blocks):
    ...
    def embed(self, x, num_samples=None):
        ...
        #print('affine_normalize:',x.shape)
        x = self.conv1(x)
        #print('init conv1:',x.shape)
        x = self.bn1(x)
        x = self.relu(x)
        if self.initial_pool:
            x = self.maxpool(x)
